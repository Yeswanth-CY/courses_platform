...
export default function ModulePage() {
  ...
  const {
    isUpdating,
    markVideoStarted,
    updateVideoProgress: trackVideoProgress,
    markVideoCompleted,
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-purple-50 via-blue-50 to-indigo-100">
        <div className="text-center bg-white rounded-2xl p-8 shadow-xl max-w-md mx-4">
          ...
          <p className="text-gray-600 mb-6">You need to be logged in to access learning content.</p>
          <Link href="/login">
  ...
  const fetchModuleData = async () => {
    if (!moduleId) {
      setError("No module selected. Please select a module from a course.")
      setLoading(false)
      return
    }
    ...
    try {
      ...
      // First try the stats endpoint, if it doesn't exist, fall back to by-id
      let response = await fetch(`/api/modules/stats?id=${moduleId}&userId=${userId}`)

      if (!response.ok) {
        // Fallback to the by-id endpoint
        response = await fetch(`/api/modules/by-id?id=${moduleId}`)
      }
      ...
      let moduleData, statsData, progressData
      ...
      if (data.module) {
        ...
        // Response from by-id endpoint
        moduleData = data
        // Calculate basic stats from the module data
        const totalVideos = moduleData.videos?.length || 0
        statsData = {
          totalVideos,
          completedVideos: 0,
          progressPercentage: 0,
          estimatedTime: totalVideos * 10, // Estimate 10 minutes per video
        }
        progressData = []
      } else {
  ...
  const fetchLearningContent = async (videoId: string) => {
    try {
      ...
      // First try video-specific content
      ...
      // Then try module-level content
      const moduleResponse = await fetch(`/api/learning-content/by-module?id=${moduleId}`)
  ...
  const updateVideoProgress = async (videoId: string, completed: boolean, progressPercentage = 100) => {
    if (!userId) return

    try {
      if (completed) {
        await markVideoCompleted(videoId, moduleId, courseId)
      } else {
        await trackVideoProgress(videoId, moduleId, courseId, progressPercentage, 0)
      }

      // Refresh module data to show updated progress
      fetchModuleData()
    } catch (error) {
        ...
        <div className="text-center bg-white rounded-2xl p-8 shadow-xl max-w-md mx-4">
          ...
          <h3 className="text-xl font-semibold text-gray-800 mb-2">
            {error === "Module not found"
              ? "Module Not Found"
              : error === "No module selected. Please select a module from a course."
                ? "No Module Selected"
          ...
          <p className="text-gray-600 mb-6">
            {error === "Module not found"
              ...
              : error === "No module selected. Please select a module from a course."
                ? "Please select a module from a course to start learning."
          ...
          <div className="flex gap-3 justify-center">
            ...
            {moduleId && <Button onClick={fetchModuleData}>Try Again</Button>}
          </div>
        </div>
      </div>
    )
